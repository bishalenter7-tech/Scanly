...
import { ArrowLeft, Share2, Scan as ScanIcon, Sparkles, ShoppingBag, AlertCircle, Quote, ShieldCheck, Bell } from 'lucide-react';
...
export default function Results() {
  ...
  // Save to compact scanHistory when result is available
  useEffect(() => {
    if (result && !scanHistory.some(item => item.productName === result.product?.name && item.timestamp > Date.now() - 60000)) {
      addScanToHistory({
        productName: result.product?.name || 'Unknown Product',
        safetyScore: result.safety_score || 0,
        safetyVerdict: result.recommendation || 'UNKNOWN',
        category: result.product_category || 'UNKNOWN',
        imageUrl: imagePreview || undefined,
      });
    }
  }, [result, addScanToHistory, scanHistory, imagePreview]);
  ...
  return (
    <div className="flex-1 max-w-5xl mx-auto w-full px-4 py-8 pb-32">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        ...
        {/* OneSignal Notification Promo Banner - First Scan Only */}
        {isFirstScan && !hidePromo && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.5 }}
            className="lg:col-span-3 p-6 bg-white border border-[#16a34a]/20 rounded-2xl shadow-sm"
          >
            <div className="flex flex-col md:flex-row items-center gap-4">
              ...
              <div className="flex-1 text-center md:text-left">
                <p className="text-[#064e3b] font-semibold">
                  Want to stay healthy? 🥗 Enable notifications for weekly food alerts and free scan refills!
                </p>
              </div>
              <div className="flex gap-3">
                <Button
                  onClick={() => setHidePromo(true)}
                  variant="outline"
      ...
      <div id="monetag-inpage-push-anchor" className="w-full flex justify-center py-6 mt-4 opacity-90"></div>
